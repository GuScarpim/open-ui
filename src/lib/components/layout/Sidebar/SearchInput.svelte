...
<div class="px-1 mb-1 flex justify-center space-x-2 relative z-10" id="search-container">
	...
	{#if focused && (filteredOptions.length > 0 || filteredItems.length > 0)}
		...
		<div
			class="absolute top-0 mt-12 left-5 right-2 border border-gray-100 dark:border-gray-900 bg-gray-50 dark:bg-gray-950 rounded-lg z-10 shadow-lg"
		...
		>
			<div class="px-2 t2 py-2 text-xs group">
				{#if filteredItems.length > 0}
					...
					<div class="max-h-60 overflow-auto">
						{#each filteredItems as item, itemIdx}
							...
							>
								...
								<div class=" text-gray-500 dark:text-gray-100/60 line-clamp-1">
									{item.id}
								</div>
							</button>
						{/each}
