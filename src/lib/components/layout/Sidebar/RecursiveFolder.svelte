	...
	>
		...
		<div class="w-full group">
			<button
				...
				class="relative w-full py-1.5 px-2 rounded-md flex items-center gap-1.5 text-xs text-gray-500 dark:text-gray-300 font-medium hover:bg-gray-100 dark:hover:bg-gray-900 transition {$selectedFolder?.id ===
				folderId
				...
				<div class="translate-y-[0.5px] flex-1 justify-start text-start line-clamp-1">
					{#if edit}
						<input
							...
							class="w-full h-full bg-transparent text-gray-500 dark:text-gray-300 outline-hidden"
						/>
					{:else}
						{folders[folderId].name}
					{/if}
